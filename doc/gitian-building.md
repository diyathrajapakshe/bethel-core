...
*Setup instructions for a Gitian build of Bethel Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bethel
...
- [Building Bethel Core](#building-bethel-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bethel-host/README.md).
...
**Note**: Bteligating in the Debian installer:
...
Clone the git repositories for bethel and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bethel/bethel-core
```
...
in the bethel repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bethel Core
----------------

To build Bethel Core (for Linux, OS X and Windows) just follow the steps under 'Setup and perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the bethel repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bethel/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bethel/bethel-core
...
For example:
```bash
URL=https://github.com/bethel/bethel-core.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bethel-core=${COMMIT} --url bethel-core=${URL} ../bethel-core/contrib/gitian-descriptors/gitian-arm.yml
./bin/gbuild --commit bethel-core=${COMMIT} --url bethel-core=${URL} ../bethel-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bethel-core=${COMMIT} --url bethel-core=${URL} ../bethel-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bethel-core=${COMMIT} --url bethel-core=${URL} ../bethel-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bethel git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bethel/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/bethel-core/bethel-detached-sigs.git

BTELPATH=/some/root/path/bethel
SIGPATH=/some/root/path/bethel-detached-sigs

./bin/gbuild --url bethel=${BTELPATH},signature=${SIGPATH} ../bethel/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bethel-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bethel-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bethel-osx-build.assert
```
...
[bethel/bethel-sigs](https://github.com/bethel/bethel-sigs) repository, or if that's not possible create a pull
request.
