...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:5556\",  (string) The bethel server IP and port we're connected to\n"
...
UniValue addanonserver(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3 ||
        ...
        throw runtime_error(
            ...
            "\nAttempts add or remove a node from the BTELTech server list.\n"
            "\nArguments:\n"
            "1. \"node\"     (string, required) The node (see getpeerinfo for nodes)\n"
    ...
    if (strCommand == "add")
    {
      if (params.size() == 3 && params[2].get_str() == "true") {
        ...
        if (it != vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_ALREADY_ADDED, "Error: BTELTech server already added");
        vAddedAnonServers.push_back(strNode);
      }
    }
    else if(strCommand == "remove")
    {
      if (params.size() == 3 && params[2].get_str() == "true") {
        ...
        if (it == vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_NOT_ADDED, "Error: BTELTech server has not been added.");
        vAddedAnonServers.erase(it);
      }
    }
    return NullUniValue;
...
UniValue getstakesubsidy(const UniValue& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "BTEL decode failed");
    }
