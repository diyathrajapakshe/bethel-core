...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total bethel balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CBethelAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"betheladdress\"\n"
            "\nReturn information about the given bethel address.\n"
            "\nArguments:\n"
            "1. \"betheladdress\"     (string, required) The bethel address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"betheladdress\", (string) The bethel address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"betheladdress\", (string) The bethel staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"betheladdress\", (string) The bethel spending address part of a cold staking address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CBethelAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CBethelAddress spendingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: Bethel address and we have full public key:
        CBethelAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are bethel addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) bethel address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CBethelAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CBethelAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"betheladdress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"betheladdress\"  (string, required) The bethel address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CBethelAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CBethelAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CBethelAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
