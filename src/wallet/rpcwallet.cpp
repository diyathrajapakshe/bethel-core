...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new Bethel address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"betheladdress\"    (string) The new bethel address\n"
            "\nExamples:\n"
    ...
    return CBethelAddress(keyID).ToString();
}
...
UniValue getcoldstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "Arguments:\n"
            "1. \"stakingaddress\"  (string, required) The bethel staking address.\n"
            "2. \"spendingaddress\" (string, required) The bethel spending address.\n\n"
            "\nExamples:\n"
    ...
    CBethelAddress stakingAddress(params[0].get_str());
    CKeyID stakingKeyID;
    if (!stakingAddress.IsValid() || !stakingAddress.GetKeyID(stakingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Staking address is not a valid Bethel address");

    CBethelAddress spendingAddress(params[1].get_str());
    CKeyID spendingKeyID;
    if (!spendingAddress.IsValid() || !spendingAddress.GetKeyID(spendingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Spending address is not a valid Bethel address");

    spendingAddress.GetKeyID(spendingKeyID);

    return CBethelAddress(stakingKeyID, spendingKeyID).ToString();
}


CBethelAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CBethelAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Bethel address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"betheladdress\"   (string) The account bethel address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Bethel address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"betheladdress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"betheladdress\"  (string, required) The bethel address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"betheladdress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"betheladdress\"  (string, required) The bethel address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"betheladdress\"  (string) a bethel address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CBethelAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CBethelAddress& address = item.first;
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            "sendtoaddress \"betheladdress\" amount ( \"comment\" \"comment-to\" \"anon-destination\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"betheladdress\"  (string, required) The bethel address to send to.\n"
            ...
            "5. \"anon-destination\"  (string, optional) Encrypted destination address if you're sending a BTELtech transaction \n"
            "6. subtractfeefromamount  (boolean, optional, default=false) The fee will be deducted from the amount being sent.\n"
            "                             The recipient will receive less bethels than you enter in the amount field.\n"
            "\nResult:\n"
    ...
    CBethelAddress address(address_str);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bethel address");
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 4)
        throw runtime_error(
            "createproposal \"betheladdress\" \"amount\" duration \"desc\" ( fee dump_raw )\n"
            "\nCreates a proposal for the community fund. Min fee of " + std::to_string((float)Params().GetConsensus().nProposalMinimalFee/COIN) + "BTEL is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"betheladdress\"     (string, required) The bethel address where coins would be sent if proposal is approved.\n"
    ...
    CBethelAddress address("NQFqqMUD55ZV3PJEJZtaKCsQmjLT6JkjvJ"); // Dummy address
    ...
    CBethelAddress destaddress(Address);
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 BTEL\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CBethelAddress address(proposal.Address);

    if(!address.IsValid())
        throw JSONRPCError(RPC_TYPE_ERROR, "Address of the proposal is not a valid Bethel address.");
    ...
    std::string Secret = sRandom + "I kindly ask to withdraw " +
            std::to_string(nReqAmount) + "BTEL from the proposal " +
...
UniValue donatefund(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "                             The fund will receive less bethels than you enter in the amount field.\n"
            "\nResult:\n"
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
...
UniValue anonsend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "anonsend \"betheladdress\" amount ( \"comment\" \"comment-to\" )\n"
    ...
    int nEntropy = GetArg("anon_entropy",BTELTECH_DEFAULT_ENTROPY);
    ...
    UniValue bteltechData = bteltech.CreateAnonTransaction(params[0].get_str(), nAmount / (nTransactions * 2), nTransactions);
    std::vector<UniValue> serverBtelAddresses(find_value(bteltechData, "anonaddress").getValues());

    if(serverBtelAddresses.size() != nTransactions)
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "BTELTech server returned a different number of addresses.");

    for(unsigned int i = 0; i < serverBtelAddresses.size(); i++)
    {
        CBethelAddress serverBtelAddress(serverBtelAddresses[i].get_str());
        if (!serverBtelAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bethel address provided by BTELTech server");
    }
    ...
    CAmount nMinAmount = find_value(bteltechData, "min_amount").get_int() * COIN;
    UniValue pubKey = find_value(bteltechData, "public_key");
    ...
    for(unsigned int i = 0; i < serverBtelAddresses.size(); i++)
    {
        ...
        if(i == serverBtelAddresses.size() - 1 || (nAmountNotProcessed - nAmountToSubstract) < (nMinAmount + 0.001))
        {
            nAmountRound = nAmountNotProcessed;
            i = serverBtelAddresses.size();
        }
        else
        {
        ...
        string encryptedAddress = bteltech.EncryptAddress(params[0].get_str(), pubKey.get_str(), serverBtelAddresses.size(), i+(i==serverBtelAddresses.size()?0:1), nId);
        wtx.strDZeel = encryptedAddress;
        SendMoney(serverBtelAddress.Get(), nAmountRound, fSubtractFeeFromAmount, wtx, encryptedAddress);
    }
...
UniValue getanondestination(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getanondestination \"betheladdress\"\n"
            ...
            "\"anonaddress\"  (string) The btel coin address to send the anon transaction to.\n"
            "\nExamples:\n"
    ...
    UniValue bteltechData = bteltech.CreateAnonTransaction(params[0].get_str());

    CBethelAddress serverBtelAddress(find_value(bteltechData, "anonaddress").get_str());
    if (!serverBtelAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bethel address provided by BTELTech server");
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"betheladdress\",     (string) The bethel address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CBethelAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CBethelAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CBethelAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"betheladdress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"betheladdress\"  (string, required) The bethel address to use for the private key.\n"
    ...
    CBethelAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"betheladdress\" ( minconf )\n"
            "\nReturns the total amount received by the given betheladdress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"betheladdress\"  (string, required) The bethel address for transactions.\n"
    ...
    // Bethel address
    CBethelAddress address = CBethelAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bethel address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tobetheladdress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a bethel address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tobetheladdress\"  (string, required) The bethel address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CBethelAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bethel address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The bethel address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less bethels than you enter in their corresponding amount field.\n"
    ...
    {
        CBethelAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Bethel address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Bethel address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of bethel addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) bethel address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"betheladdress\"  (string) A bethel address associated with the keys.\n"
    ...
    return CBethelAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    return CBethelAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CBethelAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CBethelAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CBethelAddress, tallyitem>::iterator it = mapTally.find(address);
    ...
    if (fAllAccounts || (account == strAccount))
    {
        ...
        entry.push_back(Pair("canStake", (::IsMine(*pwalletMain, r.destination) & ISMINE_STAKABLE ||
                                          (::IsMine(*pwalletMain, r.destination) & ISMINE_SPENDABLE &&
                                           !CBethelAddress(r.destination).IsColdStakingAddress(Params()))) ? true : false));
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"betheladdress\",    (string) The bethel address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"betheladdress\",    (string) The bethel address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"betheladdress\",   (string) The bethel address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending bethels\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending bethel\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"betheladdress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Bethel server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending bethels.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of bethel addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) bethel address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the bethel address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CBethelAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Bethel address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CBethelAddress(address).ToString()));
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The bethel address to receive the change\n"
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CBethelAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid bethel address");
...
UniValue resolveopenalias(const UniValue& params, bool fHelp)
{
  ...
  if ((fHelp || params.size() != 1))
      throw runtime_error(
          ...
          "\nResolves the given OpenAlias address to a Bethel address.\n"
          "\nArguments:\n"
          "1. \"address\"    (string) The OpenAlias address.\n"
          "\nExamples:\n"
          "\nGet information about an OpenAlias address\n"
          + HelpExampleCli("resolveopenalias", "\"donate@bethel.org\"")
      );
