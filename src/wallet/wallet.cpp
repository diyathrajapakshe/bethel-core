...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    if(hasPayment){
        ...
        CBethelAddress address2(address1);
    ...
    if (GetArg("-stakingaddress", "") != "" && !txNew.vout[txNew.vout.size()-1].scriptPubKey.IsColdStaking()) {
        ...
        if (stakingAddress.read(GetArg("-stakingaddress", "")))
        {
            try {
                ...
                {
                    address = CBethelAddress(find_value(addressMap, "all").get_str());
                }
                // Or use specified address if present
                if(find_value(addressMap, CBethelAddress(key.GetPubKey().GetID()).ToString()).isStr())
                {
                    address = CBethelAddress(find_value(addressMap, CBethelAddress(key.GetPubKey().GetID()).ToString()).get_str());
                }
        ...
        {
            address = CBethelAddress(GetArg("-stakingaddress", ""));
        }
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CBethelAddress(CScriptID(redeemScript)).ToString();
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign, std::string strDZeel)
{
    ...
    txNew.strDZeel = (wtxNew.strDZeel != "" ? wtxNew.strDZeel : strDZeel).length() > 0 ?
                ...
                bteltech.EncryptAddress(std::to_string(GetAdjustedTime() + (rand() % 1<<8)),sPubKey);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-bethel-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CBethelAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CBethelAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CBethelAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CBethelAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CBethelAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CBethelAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CBethelAddress(dest).ToString(), key);
}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-stakingaddress", strprintf(_("Specify a customised bethel address to accumulate the staking rewards.")));
...
std::string CWallet::formatDisplayAmount(CAmount amount) {
    ...
    string btel_amount = n.str();
    if(btel_amount.at(btel_amount.length()-1) == '.') {
        btel_amount = btel_amount.substr(0, btel_amount.size()-1);
    }
    btel_amount.append(" BTEL");
