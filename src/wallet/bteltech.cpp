...
UniValue Bteltech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    UniValue addresses = find_value(serverData, "btel_addresses");
    UniValue addrArray = addresses.get_array();

    UniValue bteltechData;
    bteltechData.setObject();
    bteltechData.pushKV("anondestination", encryptedAddress);
    bteltechData.pushKV("public_key", pubKey);
    bteltechData.pushKV("anonaddress", addrArray);
    bteltechData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    bteltechData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return bteltechData;
  } else {
    throw runtime_error("Unable to send BTELTech transaction, please try again.");
  }

}

vector<anonServer> Bteltech::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one BTELTech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid BTELTech server to send private payments.");
      return returnServers;
  }
...
UniValue Bteltech::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured BTELTech nodes are available right now.");
  }
  ...
  if (curl) {
    ...
    curl_easy_cleanup(curl);
    curl_global_cleanup();
    throw runtime_error("CURL ubtelailable");
  }
}

UniValue Bteltech::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string Bteltech::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",BTELTECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int Bteltech::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * Bteltech::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool Bteltech::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue Bteltech::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue bteltechData;
        bteltechData.setObject();
        bteltechData.pushKV("min_amount", find_value(data, "min_amount"));
        bteltechData.pushKV("max_amount", find_value(data, "max_amount"));
        bteltechData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return bteltechData;
      }
