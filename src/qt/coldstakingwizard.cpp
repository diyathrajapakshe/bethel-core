...
bool GetAddressesPage::validatePage()
{
    ...
    CBethelAddress stakingAddress(stakingAddressStr.toStdString());
    CBethelAddress spendingAddress(spendingAddressStr.toStdString());
...
void ColdStakingAddressPage::initializePage()
{
    ...
    coldStakingAddress = QString::fromStdString(CBethelAddress(stakingKeyID, spendingKeyID).ToString());

#ifdef USE_QRCODE
    QString uri = "bethel:" + coldStakingAddress;
    image->setText("");
    if(!uri.isEmpty())
    {
        // limit URI length
