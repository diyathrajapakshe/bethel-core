...
void FreespaceChecker::check()
{
    ...
    try {
        ...
        if(fs::exists(dataDir))
        {
            ...
            {
                replyStatus = ST_OK;
                replyMessage = tr("Bethel will try to import an old wallet.dat file.");
            } else {
...
void Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/bethel"));
     ...
     * override -datadir in the bethel.conf file in the default data directory
     * (to be consistent with betheld behavior)
     */
