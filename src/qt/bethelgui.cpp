...
const std::string BethelGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString BethelGUI::DEFAULT_WALLET = "~Default";

BethelGUI::BethelGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void BethelGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/bethel"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Bethel address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bethel: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Bethel addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Bethel addresses"));
    ...
    openAction->setStatusTip(tr("Open a bethel: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bethel command-line options").arg(tr(PACKAGE_NAME)));
...
void BethelGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://s3.amazonaws.com/bethel-bootstrap/bootstrap-bethel_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the Bethel Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void BethelGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(BethelUnits::Unit u, BethelUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(BethelUnits::name(u)), this);
...
void BethelGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void BethelGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        topMenuLogo->setObjectName("btelLogo");
        topMenuLogo->setStyleSheet(
           "#btelLogo { border-image: url(:/icons/menu_logo)  0 0 0 0 stretch stretch; border: 0px; }");
        ...
        // ImageButton* btelLogo2 = new ImageButton();
        // btelLogo2->setFixedSize(64,64);
        // btelLogo2->setObjectName("btelLogo");
        // btelLogo2->setPixmap(pixmap);
        //
        // toolbar->setMovable(false);
        // toolbar->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        // toolbar->addWidget(btelLogo);
        // toolbar->addWidget(btelLogo2);
...
void BethelGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool BethelGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void BethelGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool BethelGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BethelGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BethelGUI::setWalletActionsEnabled(bool enabled)
{
...
void BethelGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void BethelGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BethelGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BethelGUI::cfundProposalsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BethelGUI::cfundProposalsOpen(bool fMode)
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BethelGUI::cfundPaymentRequestsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BethelGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BethelGUI::infoClicked()
{
    if(!clientModel)
        return;

    QString link = QString("https://info.bethel.org/");
    QDesktopServices::openUrl(QUrl(link));
}

void BethelGUI::showDebugWindow()
{
...
void BethelGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BethelGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void BethelGUI::openClicked()
{
...
void BethelGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void BethelGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BethelGUI::gotoCommunityFundPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BethelGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BethelGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BethelGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BethelGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BethelGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void BethelGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Bethel network", "", count));
    if(walletFrame){
...
void BethelGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to Bethel network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to Bethel network..."));
...
void BethelGUI::showVotingDialog()
{
...
void BethelGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Bethel"); // default title
...
void BethelGUI::changeEvent(QEvent *e)
{
...
void BethelGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BethelGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BethelGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BethelUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void BethelGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BethelGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BethelGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BethelGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BethelGUI::setEncryptionStatus(int status)
{
...
void BethelGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BethelGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BethelGUI::detectShutdown()
{
...
void BethelGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BethelGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(BethelGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BethelGUI::subscribeToCoreSignals()
{
...
void BethelGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<BethelUnits::Unit> units = BethelUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const BethelUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(BethelUnits::name(unit)));
    }
...
{
    setText(BethelUnits::name(newUnits));
}
...
void BethelGUI::updatePrice()
{
  ...
  request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/btel-coin/?convert=EUR"));
...
void BethelGUI::replyFinished(QNetworkReply *reply)
{
...
void BethelGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CBethelAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void BethelGUI::getVotingInfo()
{
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.bethel.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void BethelGUI::updateStakingStatus()
{
    updateWeight();
        ...
        else if (nLastCoinStakeSearchInterval && nWeight)
        {
            ...
            if ((fFoundPaymentRequest || fFoundProposal) && !this->fDontShowAgain && (this->lastDialogShown + (60*60*24)) < GetTimeNow()) {
                ...
                if (msgbox.clickedButton()==pButtonInfo) {
                    QString link = QString("https://bethel.org/en/community-fund/");
                    QDesktopServices::openUrl(QUrl(link));
                }
            }
