    ...
    platformStyle(platformStyle)
{
    ...
    connect(ui->noBteltechButton, SIGNAL(clicked()), this, SLOT(showBtelTechDialog()));
...
void SendCoinsDialog::checkBteltechServers()
{
    bool notEnoughServers = vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1;

    ui->noBteltechLabel->setVisible(notEnoughServers);
...
void SendCoinsDialog::on_sendButton_clicked()
{
        ...
        if(entry->validate())
        {
            ...
            if(ui->anonsendCheckbox->checkState() != 0) {
                try
                {
                    ...
                    UniValue bteltechData = bteltech.CreateAnonTransaction(recipient.address.toStdString() , recipient.amount / (nTransactions * 2), nTransactions);
                    ...
                    if(serverBtelAddresses.size() != nTransactions)
                    {
                        QMessageBox::warning(this, tr("Private payment"),
                                         "<qt>" +
                                         tr("BTELTech server returned a different number of addresses.")+"</qt>");
                        valid = false;
                    }
                    ...
                    if(valid)
                    {
                      ...
                      for(unsigned int i = 0; i < serverBtelAddresses.size(); i++)
                      {
                          ...
                          cRecipient.destaddress = QString::fromStdString(serverBtelAddresses[i].get_str());
                          ...
                          cRecipient.anondestination = QString::fromStdString(bteltech.EncryptAddress(recipient.address.toStdString(), pubKey.get_str(), nTransactions, i+(i==serverBtelAddresses.size()?0:1), nId));
                          if(!find_value(bteltechData, "anonfee").isNull()){
                              cRecipient.anonfee = nAmountRound * ((float)find_value(bteltechData, "anonfee").get_real() / 100.0);
                              cRecipient.transaction_fee = find_value(bteltechData, "anonfee").get_real();
                          }else
                              valid = false;
    ...
    processSendCoinsReturn(prepareStatus,
        BethelUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    if (currentTransaction.fSpendsColdStaking && (!model->getOptionsModel()->getCoinControlFeatures() ||
        (model->getOptionsModel()->getCoinControlFeatures() && !CBethelAddress(CoinControlDialog::coinControl->destChange).IsColdStakingAddress(Params()))))
    {
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        QString amount = "<b>" + BethelUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount);
        amount.append("</b>");
        ...
        if(txFee + anonfee > 0)
        {
            ...
            questionString.append(BethelUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee + anonfee));
            ...
            if(rcp.fSubtractFeeFromAmount && anonfee > 0)
            {
                ...
                questionString.append(BethelUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), anonfee));
            }

            if(rcp.isanon){
                questionString.append("<br>" + tr("Bteltech server fee:") +QString(" ")+ QString::number(rcp.transaction_fee) + "% "+ tr(rcp.fSubtractFeeFromAmount ? "" : "(already included)") + "<br>");
                if(rcp.fSubtractFeeFromAmount)
                    questionString.append("<span style='color:#aa0000;'>" + BethelUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount * ((rcp.transaction_fee/100))) + "</span> " + tr("will be deducted as Bteltech fee.") + "<br>");
    ...
    Q_FOREACH(BethelUnits::Unit u, BethelUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(BethelUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1")
        .arg(BethelUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::showBtelTechDialog()
{
    bteltechsetup* setupBtelTech = new bteltechsetup();
    setupBtelTech->setWindowIcon(QIcon(":icons/bethel"));
    setupBtelTech->setStyleSheet(Skinize());
                                 ...
                                 const CAmount& coldStakingBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(BethelUnits::formatWithUnit(0, balance) + (model->getOptionsModel()->getDisplayUnit() != 0 ?( " (" + BethelUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance) + ")") : ""));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(BethelUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::minimizeFeeSection(bool fMinimize)
{
    ...
    ui->noBteltechButton  ->setVisible(fMinimize);
    //ui->horizontalLayoutSmartFee->setContentsMargins(0, (fMinimize ? 0 : 6), 0, 0);

    if(fMinimize)
        checkBteltechServers();
    else
        ui->noBteltechLabel  ->setVisible(false);
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
  ...
  //  else {
  //      ui->labelFeeMinimized->setText(BethelUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
            BethelUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000)) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(BethelUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CBethelAddress addr = CBethelAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Bethel address"));
        }
        else // Valid address
        {
            bool fHaveKey = false;
