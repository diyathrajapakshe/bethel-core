...
bool WalletModel::validateAddress(const QString &address)
{
  ...
  CBethelAddress addressParsed(address_str);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered bethel address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CBethelAddress(rcp.isanon ? rcp.destaddress.toStdString() : rcp.address.toStdString()).Get());
        ...
        Q_FOREACH(const CRecipient &rcp, transaction.vecSend)
        {
          ...
          Q_FOREACH(const CWalletTx newTx, transaction.vTransactions)
          {
            ...
            Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
            {
              ...
              else if (!rcp.message.isEmpty()) // Message from normal bethel:URI (bethel:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CBethelAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CBethelAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CBethelAddress(sAddress).Get();
