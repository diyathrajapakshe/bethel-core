...
const int BETHEL_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BETHEL_IPC_PREFIX("bethel:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
const char* NIP01_MESSAGE_SIGNEDMSG = "SignedMsg";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/bethel-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/bethel-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/bethel-paymentrequest";
const char* NIP01_MIMETYPE_SIGNEDMSG = "application/bethel-signedmsg";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the bethel: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BETHEL_IPC_PREFIX, Qt::CaseInsensitive)) // bethel: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseBethelURI(arg, &r) && !r.address.isEmpty())
            {
                CBethelAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BETHEL_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(BETHEL_IPC_CONNECT_TIMEOUT);
    ...
    model(0)
{
    ...
    // on Mac: sent when you click bethel: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            LogPrintf("Cannot start bethel: click-to-pay handler\n");
        }
        else {
...
//
// OSX-specific way of handling bethel: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in bethel: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bethel://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bethel://' is not a valid URI. Use 'bethel:' instead."),
                       CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BETHEL_IPC_PREFIX, Qt::CaseInsensitive)) // bethel: URI
    {
        ...
        if (uri.hasQueryItem("m") && uri.hasQueryItem("a")) // sign message URI
        {
            ...
            CBethelAddress addr(uri.queryItemValue("a").toStdString());
            if (!addr.IsValid())
            {
            ...
            QUrl fetchUrlConstructed(s.mid(BETHEL_IPC_PREFIX.length()));
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBethelURI(s, &recipient))
            {
              ...
              if (!address.IsValid()) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Bethel address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CBethelAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom bethel addresses are not supported
        ...
        // Bethel amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(BethelUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
