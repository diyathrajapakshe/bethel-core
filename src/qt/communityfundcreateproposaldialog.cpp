...
{
    ui->setupUi(this);
    GUIUtil::setupAddressWidget(ui->lineEditBethelAddress, this);
...
{
    bool isValid = true;
    if(!ui->lineEditBethelAddress->isValid() || (ui->lineEditBethelAddress->text() == QString("")))
    {
        // Styling must be done manually as an empty field returns valid (true)
        ui->lineEditBethelAddress->setStyleSheet(STYLE_INVALID);
        ui->lineEditBethelAddress->setValid(false);
        isValid = false;
    }
...
void CommunityFundCreateProposalDialog::click_pushButtonCreateProposal()
{
    ...
    if(this->validate())
    {
        ...
        // Address
        string Address = ui->lineEditBethelAddress->text().toStdString().c_str();
        ...
        if (curBalance <= Params().GetConsensus().nProposalMinimalFee) {
            QMessageBox msgBox(this);
            string fee = std::to_string(Params().GetConsensus().nProposalMinimalFee / COIN);
            std::string str = "You require at least " + fee + " BTEL mature and available to create a proposal\n";
            msgBox.setText(tr(str.c_str()));
            msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
            msgBox.setIcon(QMessageBox::Warning);
            msgBox.setWindowTitle("Insufficient BTEL");
            msgBox.exec();
            return;
        }
