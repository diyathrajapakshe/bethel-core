        ...
        if(pcoinsTip->GetAllPaymentRequests(mapPaymentRequests))
        {
            for (CPaymentRequestMap::iterator it_ = mapPaymentRequests.begin(); it_ != mapPaymentRequests.end(); it_++)
            {
                ...
                if(prequest.fState == CFund::ACCEPTED && prequest.paymenthash == uint256() &&
                    ...
                    CFund::CProposal proposal;
                    if(pcoinsTip->GetProposal(prequest.proposalhash, proposal)) {
                        CBethelAddress addr(proposal.Address);
                        if (!addr.IsValid())
                            continue;
                        coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
...
{
    LogPrintf("BethelStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bethel-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in BethelStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running BethelStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BethelStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BethelStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("BethelStaker: ProcessNewBlock, block not accepted");
        }
        else
        {
