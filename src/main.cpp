...
// Copyright (c) 2017-2018 The Bethel Core developers
...
#if defined(NDEBUG)
# error "Bethel cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Bethel Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight == 1)
    {
        CAmount nSubsidy = 150000000 * COIN;
        return nSubsidy;
    }
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        if (!inputs.HaveInputs(tx))
            return state.Invalid(false, 0, "", "Inputs ubtelailable");
        CAmount nValueIn = 0;
        CAmount nFees = 0;
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
        ...
        if (fAddressIndex)
        {
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
                    ...
                    CBethelAddress address(destination);
    ...
    for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
        ...
        if(nAccValue > nPOWBlockReward && block.vtx[0].vout[i].nValue > 0) {
            ...
            if(metadata[nPaymentRequestsCount].isStr())
            {
                ...
                if(block.vtx[0].vout[i].nValue != prequest->nAmount || prequest->fState != CFund::ACCEPTED || proposal.Address != CBethelAddress(address).ToString())
...
//
// Commented - Bethel uses now version control
//
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of Bethel, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of Bethel, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of Bethel, please update.";
            fObsolete = true;
        }
