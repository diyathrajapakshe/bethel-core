...
const char * const BETHEL_CONF_FILENAME = "bethel.conf";
const char * const BETHEL_PID_FILENAME = "bethel.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bethel
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bethel
    // Mac: ~/Library/Application Support/Bethel
    // Unix: ~/.bethel
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bethel4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bethel4";
#else
    // Unix
    return pathRet / ".bethel4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", BETHEL_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No bethel.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override bethel.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", BETHEL_PID_FILENAME));
