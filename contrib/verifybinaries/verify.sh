...
###   This script attempts to download the signature file SHA256SUMS.asc from bethel.org
...
WORKINGDIR="/tmp/bethel"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://bethel.org/bin/"
VERSIONPREFIX="bethel-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bethel-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bethel Core binary release signing key installed?"
   fi
