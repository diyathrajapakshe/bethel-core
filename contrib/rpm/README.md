...
The RPM spec file provided here is for Bethel-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bethel.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bethel git repository](https://github.com/bethel/bethel) but are not part of
...
Currently only `Source10` is used, for the example `bethel.conf` file.
...
in the bethel git repository. At present only an SVG version of the Bethel
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bethel
...
needed when 0.12.1 is released, a proper fix is already in the Bethel git
master branch.
...
idea to build Bethel Core against a static version of that library compiled
...
The `bethel.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bethel-server` package. At this point, most RPM based
...
The `bethel-server` package should have SELinux support. How to properly do
...
Bethel-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bethel/). He has been
packaging Bethel for Fedora at least since 2012.
...
4. I renamed the `bethel` package that contains the Qt GUI to `bethel-core` as
that appears to be how the general population refers to it, in contrast to
`bethel-xt` or `bethel-classic`. I wanted to make sure the general population
