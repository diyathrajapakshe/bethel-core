...
class AbandonConflictTest(BethelTestFramework):
    def __init__(self):
        super().__init__()
        self.num_nodes = 2
    ...
    def run_test(self):
        ...
        # Identify the 10BTEL outputs
        nA = next(i for i, vout in enumerate(self.nodes[0].getrawtransaction(txA, 1)["vout"]) if vout["value"] == Decimal("10"))
        nB = next(i for i, vout in enumerate(self.nodes[0].getrawtransaction(txB, 1)["vout"]) if vout["value"] == Decimal("10"))
        nC = next(i for i, vout in enumerate(self.nodes[0].getrawtransaction(txC, 1)["vout"]) if vout["value"] == Decimal("10"))

        inputs =[]
        # spend 10BTEL outputs from txA and txB
        ...
        # Identify the 14.99998BTEL output
        ...
        # Verify that B and C's 10 BTEL outputs are available for spending again because AB1 is now conflicted
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance + Decimal("20"))
        balance = newbalance

        # There is currently a minor bug around this and so this test doesn't work.  See Issue #7315
        # Invalidate the block with the double spend and B's 10 BTEL output should no longer be available
